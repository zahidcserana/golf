...
class GolfCourse extends Model
{
 	...
 	public function DeleteCourse($courseId)
	{
 	...
 	public function GetState($userId)
 	{
		...
		{
			if($key=='fir_percentage' && $statAverage['hash_par4']['sum'] + $statAverage['hash_par5']['sum']!=0 )
				$aStat['avg'] = round( $statAverage['fairways']['sum'] * 100 / ( $statAverage['hash_par4']['sum'] + $statAverage['hash_par5']['sum']), 2);
			else if($key=='gir_percentage' && $statAverage['hash_par3']['sum']+$statAverage['hash_par4']['sum'] + $statAverage['hash_par5']['sum']!=0)
